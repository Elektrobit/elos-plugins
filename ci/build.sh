...
CMAKE_PARAM="${CMAKE_PARAM} \
    -DCMAKE_PREFIX_PATH=${CMAKE_PREFIX_PATH:+"${CMAKE_PREFIX_PATH};"}${DEPS_DIR}"
CMAKE_PARAM="${CMAKE_PARAM} -DCMAKE_INSTALL_PREFIX:PATH=${PREFIX_PATH}"
export LIBRARY_PATH="${LIBRARY_PATH:+"${LIBRARY_PATH}:"}${DEPS_DIR}/lib"
export BINDGEN_EXTRA_CLANG_ARGS="-I${DEPS_DIR}/include"
