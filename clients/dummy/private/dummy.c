...
static safuResultE_t _pluginLoad(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        if ((plugin->config == NULL) || (plugin->config->key == NULL)) {
            ...
            safuLogDebugF("Dummy Client Plugin '%s' has been loaded", plugin->config->key);
...
static safuResultE_t _pluginStart(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        safuLogDebugF("Dummy Client Plugin '%s' has been started", plugin->config->key);
...
static safuResultE_t _pluginStop(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        safuLogDebugF("Stopping Dummy Client Plugin '%s'", plugin->config->key);
...
static safuResultE_t _pluginUnload(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        safuLogDebugF("Unloading Dummy Client Plugin '%s'", plugin->config->key);
        free(plugin->data);
        result = SAFU_RESULT_OK;
    }
